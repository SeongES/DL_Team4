...
class RiskDetector:
    def __init__(self, yolo_model_path, encoder_path, decoder_path, gt_depth_path,
                 labels_json, device='cuda', speaker = None):
    ...
    def calculate_risk_score_frame(self, frame):
        ...
        for i, bbox in enumerate(bbox_tensor):
            ...
            location = 'left' if center_x < original_width / 3 else 'front' if center_x < 2 * original_width / 3 else 'right'
...
def run_video_analysis(video_path, save_path=None):
    ...
    while cap.isOpened():
        ...
        if best_obj:
            ...
            class_name, location, risk_score , min_depth, _ = best_obj
            if risk_score > 10.0:

                if not alarm_active:
                    detector.speaker.update(class_name, location)
                    alarm_active = True
